...
//   //This is Actually the Main HomePage
//   //Creating an Array of data that has to be Rendered within the divs grid
//  const [headings_arr, setHeadingsArr] = useState(["PUNJABI UNIVERSITY PATIALA", "", "", "D.C. MODEL SR. SEC. SCHOOL"]);
//  const [subHeadingsArr, setSubHeading] = useState(["B.Tech(Computer Science & Engineering", "", "", "High School Matriculation"]);
//  const [description_arr, setDescription] = useState(["Sixth Semester Student, familiar with Coursework of Data Structures & Algorithms, Relational Databases, Computer Networks, Web Fundamentals, Computational Theory & System Architecture aimed at Developing strong analytical and problem-solving skills to design efficient systems", "", "", "Completed primary education till 10th grade and senior secondary education in the PCM stream at D.C. Model Sr. Sec. School. Led the Spotlight Club, showcasing excellence in debating, dramatics, and theatre, while fostering leadership, creativity, and communication skills"]);
//  const [default_styling, setDefaultStyling] = useState("grid_item border border-gray-900 h-[38vh]");
//  const [styling_odd_div, setStylingOddDiv] = useState("grid_item border border-gray-900 h-[38vh] text-start");
 
//  //If we Declare this Array normally without using UseState, it shows "Not Defined" Error
  
//   return (
//    <>
//   <div className="full_div w-[100vw] h-[120vh] bg-gray-900">
//     <div className="navigator_tab flex mx-auto gap-3 w-fit border border-gray-950 rounded-3xl p-1">
//       <h1 className="education border border-green-400 rounded-3xl bg-green-600 p-3">
//         Education 
//       </h1>
//       <h1 className="experience border border-gray-900 rounded-3xl p-3 text-slate-50">
//         Experience
//       </h1>
//     </div>
//     {/* <FontAwesomeIcon icon={faGraduationCap} className='relative top-[4%] right-[50%] left-[49%] text-gray-400 scale-150'/>; */}
//     <div className="externalmost_div bg-gray-900 w-[95vw] h-[85vh] grid grid-cols-2 mx-auto mt-4 p-4">
//       {headings_arr.map((heading, i)=>(
        
//       <div className={`grid_item p-3 ${i == 1 ? "border-l-2 border-l-green-800 w-[42vw]" : ""} ${i == 2 ? "border-r-2 border-r-green-800 w-[46.4vw]" : ""} ${i == 0 ? "border-2 border-green-800 rounded-lg w-[42vw] shadow-[0_0_10px_0_#166534]" : ""} ${i == 3 ? "border-2 border-green-800 rounded-lg w-[42vw] ml-6 shadow-[0_0_10px_0_#166534]" : ""} h-[38vh] flex-col ${i == 0 || i == 3 ? "text-end" : "text-start"}`}>
//           <h1 className='text-slate-50 font-extrabold font-serif'> {heading} </h1> 
//           <h3 className='text-slate-50 font-bold font-serif'> {subHeadingsArr[i]} </h3>
//           <p className='text-gray-500 font-mono'> {description_arr[i]} </p>
//           <div className={`circular border border-black rounded-3xl w-[1.5vw] bg-green-800 h-[3vh] ${i == 1 ? "absolute top-[15.2%] right-[48.5%]" : ""} ${i == 2 ? "absolute top-[55%] right-[48.5%]" : ""} ${i == 1 || i == 2 ? "visible" : "invisible"}`}> </div>
//         </div>
//       ))};
//     </div>
//     </div>
//    </>
//   )
//   //Lesson 01: map function ke andar agar Curly Braces lagaoge, toh JSX Explicitly Return krni padegi
//   //therefore, write logic within () in the map method 
...
export default function Home() {
  ...
  const experienceData = [
    {title: "Web Developer Intern @ Zymo",
    description: "Interned as Full-Stack Intern with Mumbai-based Car-Rentals Startup Zymo"
  },
    {
  ...
  const [timeline_arr, setTimeline] = useState(["February 2025 - Present","June 2024 - August 2024", "June 2023 - July 2023", "October 2023 - October 2024"]);
const [skills_data, setSkillsData] = useState([{skill1: "React.Js", skill2: "Tailwind CSS", skill3: "Node.Js", skill4: "Express.Js"}, {skill1 : "JavaScript", skill2: "CSS", skill3: "HTML"}, {skill1: "Communication", skill2: "Marketing"}, {skill1: "Leadership", skill2: "Public Speaking", skill3: "Public Relations"}]);
...
const [workex_state, setWorkExState] = useState("Recent to Oldest");
  ...
  const sortExperiences = () => {
    // setWorkExperiences([...workExperiences].reverse()); 
    ...
    // const shallow_timeline_arr = [...timeline_arr];
    // setTimeline(shallow_timeline_arr.reverse());


    console.log("Before Reversal:", timeline_arr);

    setWorkExperiences([...workExperiences].reverse());
    setSkillsData([...skills_data].reverse());

    const reversedTimeline = [...timeline_arr].reverse();
    console.log("After Reversal:", reversedTimeline);

    setTimeline(reversedTimeline);
  ...
  return (
    <>
      <div className="external_holder_div h-[190vh] w-[100vw] border-b-2 flex-col justify-evenly">
{/*          
            <div className="scroll_logo absolute left-[85%] flex justify-center items-center">
              <box-icon name="down-arrow" type="solid" color="#391e99" size="md"></box-icon>
            </div> */}
         ...
         <div className="external_bordering_div border-l-4 border-l-black w-[80vw] mx-auto" key={index}>
          ...
          <FontAwesomeIcon icon={faBriefcase} className="scale-[1.75] my-auto ml-4" />
            <div className="text_info w-[90%] h-5/6 flex-col" key={index}>
              ...
              <div className="skills_used h-full w-full flex items-center justify-around mt-4" key={index}>
                ...
                <div className={`skill_item font-bold ${skills_data[index].skill4 ? "border-2 border-black rounded-lg p-1" : ""}`}> {skills_data[index].skill4} </div>
